...
# NOTE: plonky2 release build takes 15+ minutes — skip in CI to avoid timeouts
...
if [ "${CI:-}" = "true" ] || [ "${GITHUB_ACTIONS:-}" = "true" ]; then
    echo -e "${YELLOW}  ⚠ Skipping ZKP prover build in CI (takes 15+ min, sidecar uses mock receipts)${NC}"
else
